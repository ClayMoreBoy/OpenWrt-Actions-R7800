...
cp -f ../zzz-default-settings-Snapshot package/lean/default-settings/files/
...
sed -i 's/IMG_PREFIX:=$(VERSION_DIST_SANITIZED)/IMG_PREFIX:=ClayMoreBoy-$(TZ=UTC-8 date +%Y%m%d)-$(VERSION_DIST_SANITIZED)/g' include/image.mk
